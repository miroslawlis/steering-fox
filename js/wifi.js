...
window.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById('wifiDisconnect').addEventListener('click', function () {
        wifiDisconnect();
    });
    document.getElementById('wifiConnect').addEventListener('click', function () {
        wifiConnect();
    });
    document.getElementById('wifiRefreshList').addEventListener('click', function () {
        wifiInfo();
    });
    ...
    if (isLinux) {
        ...
        try {
            ...
            executeCommand('iw wlan0 link | grep SSID', function (SSID) {
                ...
                executeCommand("iwlist wlan0 scan | grep ESSID", function (wifiListOutput) {
                    ...
                    document.querySelector('.settings .wifi .currentNetwork').innerHTML = '<p>Connected to: ' + currentWifi + '</p>';
                    // clear DOM object
                    document.querySelector('.settings .wifi .availableNetworks .data').innerHTML = '';

                    wifiNetworkList.forEach(element => {
                        document.querySelector('.settings .wifi .availableNetworks .data').innerHTML += '<div class="ssid" onclick="selectedWiFitoConnect(this);">' + element + '</div>';
                    });
        ...
        try {
            // current wifi SSID
            executeCommand('netsh wlan show interface name="Wi-Fi" | findstr "\\<SSID\\>"', function (SSID) {
                // list o available networks
                executeCommand('netsh wlan show networks | findstr "SSID"', function (wifiListOutput) {
                    currentWifi = SSID;
                    currentWifi = currentWifi.replace(/ |S|I|D|:|\n/g, "");
                    // trimRight removes white spaces and new lines from end
                    wifiNetworkList = wifiListOutput.trimRight();
                    wifiNetworkList = wifiNetworkList.replace(/SSID [0-9] : /g, '').replace(/\n+/g, "\n");
                    wifiNetworkList = wifiNetworkList.split("\n");
    ...
    if (isLinux) {
        ...
        try {
            ...
            executeCommand('netsh wlan disconnect', function (output) {
                debugLog("disconected from Wi-Fi network: " + currentWifi);
                document.querySelector('.settings .wifi .currentNetwork').innerHTML = '<p class="warning">No connection</p>';
            });
...
function wifiConnect(e) {
    ...
    let selectedWiFitoConnect = document.querySelector('.settings .wifi .data .ssid.selectedWiFi');
    let wifi_password;
    ...
    if (isLinux) {
        ...
        try {
            ...
            executeCommand('wpa_cli add_network', function (output_Network_id) {
                ...
                // 
                executeCommand('wpa_cli set_network ' + output_Network_id + ' ssid \'"' + selectedWiFitoConnect.innerText + '"\' psk \'"' + wifi_password + '"\'', function (output_wpa_cli) {
                    debugLog('output_wpa_cli: ' + output_wpa_cli);
                    document.querySelector('.settings .wifi .currentNetwork').innerHTML = '<p>Connected to: ' + currentWifi + '</p>';
                    debugLog("conected to Wi-Fi network: " + selectedWiFitoConnect.innerText);
                });
        ...
        try {
            // connect to WiFi network
            executeCommand('netsh wlan connect name="' + selectedWiFitoConnect.innerText + '"', function (output) {

                document.querySelector('.settings .wifi .currentNetwork').innerHTML = '<p>Connected to: ' + currentWifi + '</p>';
                debugLog("conected to Wi-Fi network: " + selectedWiFitoConnect.innerText);

                let stringHelper = 'There is no profile "' + selectedWiFitoConnect.innerText + '" assigned to the specified interface.'
...
function selectedWiFitoConnect(element) {
    document.querySelectorAll('.settings .wifi .availableNetworks .data .selectedWiFi').forEach(ele => {
