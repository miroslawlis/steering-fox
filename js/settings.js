...
var settings_app = {
  ...
  check_for_settings_file_create() {
    ...
    try {
      ...
      if (
        !fs.existsSync(this.filePathAndName) ||
        fs.readFileSync(this.filePathAndName).length < 1
      ) {
        ...
        fs.writeFile(
          ...
          JSON.stringify(settingsFileContent),
          (err) => {
            if (err) {
  ...
  listen_for_UI_changes() {
    ...
    document
      ...
      .addEventListener(
        "volumechange",
        () => settings_app.saveSettingsToFile(true),
        false
      );
  ...
  readSettingfile() {
    ...
    try {
      ...
      this.updateGUIwithSettings(
        settingFromFileObj.themeOptionFile,
        settingFromFileObj.musicFolder,
        settingFromFileObj.audioVolume
      );
      //
      switchTheme();
      createSongsObject();
  ...
  updateGUIwithSettings(
    themOpt = document.querySelector(".settings .themeOption"),
    fileOpt = document.querySelector(".settings .item .filepath"),
    audioOpt = audio.volume
  ) {
    // update GUI
    let themeElement = document.querySelector(".settings .themeOption");
