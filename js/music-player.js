...
var audioVolBefore, ShuffledSongsObject, currentSongID, nextSongID, currentSongIndex, nextSongIndex, prevSongID, prevSongIndex, curentSongNumberInObj;
...
function playAudio(forcePlay) {
    ...
    if (document.querySelector('#music-player .play.button').classList.contains('active') && forcePlay !== true) {
...
function markCurentSongInTable() {
    ...
    let rowsWithClass = document.querySelectorAll('#wrapperMtable .curentlyPlaying');
    rowsWithClass.forEach((row) => {
        row.classList.remove('curentlyPlaying');
    });
    //add class curentlyPlaying to one row
    document.querySelector('.itemIDrow' + audio.getAttribute('itemid')).classList.add('curentlyPlaying');
