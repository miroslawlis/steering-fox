...
function modalClose(element) {
    ...
    if ( elParentToClose.classList.contains(modalClasses[0]) && elParentToClose.classList.contains(modalClasses[1]) ) {
...
document.onclick = function (event) {
    ...
    if (event.target.classList.contains(modalClasses[0]) && event.target.classList.contains(modalClasses[1]) ) {
        event.target.remove();
    }
